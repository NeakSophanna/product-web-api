...
public class MyController {
	...
	public Object getAll() {
		...
		if(product.isEmpty()) {
			return ResponseEntity.status(404).body("Product is empty");
		}
	...
	public Object postProduct(@ModelAttribute PostProductDAO product)throws Exception {
		...
		String extensions = Objects.requireNonNull(file.getOriginalFilename());
		String fileName = UUID.randomUUID()+"-"+extensions;
	...
	public Object getByName(@PathVariable("name") String name) {
		List<Product> pro = productRepo.getByName(name);
		if(pro.isEmpty())
			return ResponseEntity.status(404)
	...
	public Object putProduct(@ModelAttribute PutProductDAO product)throws Exception {
		...
		if(p.isPresent()) {
			...
			return ResponseEntity.status(HttpStatus.ACCEPTED)
					.body(Map.of("message","Product Id: "+ id +" has been updated",
								"product",update));
		}
		return ResponseEntity.status(HttpStatus.NOT_FOUND)
				.body(Map.of("message","Product Id: "+id +" not found"));
